...
<html lang="en">
 ...
 <body>
   ...
   <hr>
		...
		<?php
			$result=mysql_query("DELETE FROM announcements WHERE author = '" . ($_GET['author']). "'") or die(mysql_error());
			
			$result=mysql_query("SELECT * FROM announcements") or die(mysql_error());

			while($row = mysql_fetch_array($result))
			  {
			  echo $row['title'] . "<br>". $row['details']. "<br>" . "posted by: " . $row['author'] . ", " . $row['vdate'];
			  }
		?>
